...
(async function () {
    ...
    let stores = [];
    // Try fetching stores from backend API first
    try {
        const resp = await fetch('/api/stores?limit=200');
        if (resp.ok) {
            const data = await resp.json();
            if (Array.isArray(data) && data.length) {
                stores = data.map(s => ({ id: s._id || s.id, name: s.name, lat: s.lat || s.locationLat || s.latitude || 0, lng: s.lng || s.locationLng || s.longitude || 0, address: s.address || s.location || '', rating: s.rating || '', hours: s.openTime ? `${s.openTime} - ${s.closeTime || ''}` : '', img: s.image || s.logo || 'https://via.placeholder.com/120x80/00703c/ffffff?text=STORE' }));
        }
    }
    } catch (err) {
        console.warn('API stores fetch failed, will try DOM/template fallback', err);
    }

    // If we didn't get stores from API, try parsing template in DOM
    ...
    // If parsing failed or the template was empty, we will not seed sample stores here.
    // The app must rely on the backend API; show an empty map if none available.
    if (!Array.isArray(stores) || stores.length === 0) {
        console.warn('No stores found from API or template — map will be empty');
        stores = [];
    }
            ...
            <div class="popup-card">
                ...
                <div class="popup-body">
                    ...
                    <div style="margin-top:6px">
                        <a href="store-page.html?storeId=${encodeURIComponent(store.id)}" class="popup-link">View store</a>
                        &nbsp;|&nbsp;
    ...
    try {
        ...
        if (r.ok) {
            ...
            if (Array.isArray(j.products)) productNames = Array.from(new Set(j.products.map(p => p.name)));
        }
    } catch (e) {
        // fallback to local helper
        const productList = (typeof getAllProducts === 'function') ? getAllProducts() : [];
        productNames = Array.from(new Set(productList.map(p => p.name)));
    }
...
// initialize global header/helpers if available
